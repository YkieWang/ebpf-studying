...
## 例2 <a href="#jiao-shou-jia-wen-jian-ju-li-2" id="jiao-shou-jia-wen-jian-ju-li-2"></a>
...
struct xdp_sample_bpf {
	...
	struct xdp_sample_bpf__bss {
		int cpumap_map_id;
	} *bss;
	struct xdp_sample_bpf__rodata {
		int nr_cpus;
		int from_match[32];
		int to_match[32];
	} *rodata;
...
可以看到其中的多了maps/bss/rodata，同理也可以通过obj->maps.rx\_cnt来操作想要的maps。

完整文件见附件，这里仅列出了一部分进行学习：

{% file src="../.gitbook/assets/xdp_sample.skel.h" %}
